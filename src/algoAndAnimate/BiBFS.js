...
const runBiBFS = (start, end, row) => {
  ...
  const isValid1 = (xx, yy) => {
    if (xx >= 0 && yy >= 0 && xx < row && yy < row) {
      if (
        visited1[xx][yy] === false &&
        [xx, yy].toString() !== start.toString()
      ) {
        return true;
      }
    }
  ...
  const isValid2 = (xx, yy) => {
    if (xx >= 0 && yy >= 0 && xx < row && yy < row) {
      if (
        visited2[xx][yy] === false &&
        [xx, yy].toString() !== end.toString()
      ) {
        return true;
      }
    }
  ...
  dq1.enqueue([
    start,
    JSON.stringify([String(start[0] + "," + String(start[1]))]),
  ]);
    ...
    for (i = 0; i < dir.length; i++) {
      ...
      if (isValid1(new_x1, new_y1)) {
        if (visited2[new_x1][new_y1]) {
          animate(
            res,
            path1.concat(JSON.parse(pathStore2[idName1])),
            start,
            end
          );
          return;
      ...
      if (isValid2(new_x2, new_y2)) {
        if (visited1[new_x2][new_y2]) {
          animate(
            res,
            path2.concat(JSON.parse(pathStore1[idName2])),
            start,
            end
          );
          return;
  ...
  document.getElementById("noPath").className +=
    " mt-3 border border-rounded p-2";
  return;
};
