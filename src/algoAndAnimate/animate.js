const animate = (colorArray, shortestPath, start, end) => {
  ...
  for (let i = 0; i < colorArray.length; i++) {
    setTimeout(() => {
      if (colorArray[i] !== startId && colorArray[i] !== endId) {
        document.getElementById(colorArray[i]).className =
          "spinner-grow text-danger bg-dark border";
        document.getElementById(colorArray[i]).role = "status";
        document.getElementById(colorArray[i]).innerHTML =
  ...
  document.getElementById("numOfNodesTraversed").innerHTML =
    "Num of nodes traversed:" + colorArray.length;
  document.getElementById("numOfNodesShortestPath").innerHTML =
    "Num of nodes in shortest path:" + shortestPath.length;
};
